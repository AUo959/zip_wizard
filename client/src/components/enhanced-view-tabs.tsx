...
export function EnhancedViewTabs({
  ...
  // Handle tab click
  const handleTabClick = useCallback((view: ViewType) => {
  ...
  const renderTab = useCallback((view: ViewType, index: number) => {
      ...
      >
        ...
        {shortcutNumber && (
          <span className="sr-only">
            Press {shortcutNumber} to activate
          </span>
        )}
  ...
  const renderGroupedTabs = useCallback(() => {
    ...
    return Object.entries(groupedViews).map(([category, categoryViews]) => {
      const visibleCategoryViews = categoryViews.filter(view => 
        visibility[view]?.hidden !== true
      );
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div 
          ...
          className="enhanced-nav__tablist"
        >
  ...
  const renderMobileNavigation = () => (
    <nav 
      className="enhanced-nav enhanced-nav__mobile"
      aria-label="Main navigation"
    >
      <div className="enhanced-nav__dropdown">
        ...
        {isDropdownOpen && (
          <div 
            role="tablist"
            className="enhanced-nav__dropdown-content"
            onKeyDown={handleKeyDown}
          >
            {groupedViews ? renderGroupedTabs() : visibleViews.map(renderTab)}
          </div>
