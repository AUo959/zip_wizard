 ...
 */
export async function* streamFileChunks(file: File, chunkSize = 1024 * 1024): AsyncGenerator<ArrayBuffer> {
  let offset = 0;
 ...
 */
export function streamFileWithWebStreams(file: File, chunkSize = 1024 * 1024): ReadableStream<Uint8Array> {
  let offset = 0;
 ...
 */
export function estimateMemoryUsage(fileSize: number, chunkSize: number): {
  peakMemory: number;
